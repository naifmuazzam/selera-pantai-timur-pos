const MenuCard = ({ item, onAdd, isAddOn }) => {
  return (
    <div className={`bg-white rounded-xl shadow-md overflow-hidden flex flex-col ${isAddOn ? "border-2 border-dashed border-amber-200" : ""}`}>
      <div className={`w-full ${isAddOn ? "h-20" : "h-32"} flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100`}>
        <span className={`${isAddOn ? "text-3xl" : "text-5xl"}`}>{item.emoji}</span>
      </div>
